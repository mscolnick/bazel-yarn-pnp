...
export class LocalTarballFetcher implements Fetcher {
        ...
        onMiss: () =>
          opts.report.reportCacheMiss(
            ...
            `${structUtils.prettyLocator(
              ...
              locator
            )} can't be found in the cache and will be tar->zip from the local repository rules.`
          ),
  ...
  private getLocatorUrl(locator: Locator, opts: MinimalFetchOptions) {
    return `${process.env.WORKSPACE}/${locator.name}`;
  }
}
